...
class LinearSortFunction: BaseDistancedSortFunction {
    let direction: SpruceDirection
    
    init(direction: SpruceDirection, interObjectDelay: TimeInterval) {
        self.direction = direction
        super.init(interObjectDelay: interObjectDelay)
    }
    
    override func getDistanceBetweenPoints(left: CGPoint, right: CGPoint) -> Double {
        var left = left
        var right = right
        switch direction {
