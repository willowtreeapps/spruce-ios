...
class CorneredSortFunction: BaseDistancedSortFunction {
    
    let corner: SpruceCorner
    
    init(corner: SpruceCorner, interObjectDelay: TimeInterval) {
        self.corner = corner
        super.init(interObjectDelay: interObjectDelay)
    }
    
    override func getTimeOffsets(view: UIView) -> [SpruceTimedView] {
